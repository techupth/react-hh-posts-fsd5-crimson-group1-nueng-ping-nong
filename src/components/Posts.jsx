...
function Posts() {
  const [postItems, setPostItems] = useState(postContent);

  const likePost = (postItemsIndex) => {
    const newPostItems = [...postItems];
    newPostItems[postItemsIndex].likes += 1;
    setPostItems(newPostItems);
  };

  const disLikePost = (postItemsIndex) => {
    const newPostItems = [...postItems];
    if (newPostItems[postItemsIndex].likes > 0) {
      newPostItems[postItemsIndex].likes -= 1;
      setPostItems(newPostItems);
    } else {
      return;
